...
- [X] T001 Update `build.gradle` dependencies: add `spring-boot-starter-web`,
...
- [X] T002 Add the `org.openapi.generator` Gradle plugin in `build.gradle`; copy the contract to
...
- [X] T003 Apply the `jacoco` plugin in `build.gradle`: configure `jacocoTestReport` (HTML+XML) and
...
- [X] T004 [P] Configure `src/main/resources/application.yaml` for H2 (datasource + JPA `ddl-auto`)
  and enable the H2 console.
- [X] T005 [P] Create the Clean Architecture package skeleton under
...
- [X] T006 [P] Create domain exception types in `src/main/java/.../domain/exception/`:
  `DomainException`, `EntityNotFoundException`, `DependentRecordsException` (for FR-013 conflicts),
  `ValidationException`.
- [X] T007 Create global exception handling in
...
- [X] T008 [P] Create a shared timestamp/identity strategy in
...
- [X] T009 [P] [US1] Functional Gherkin scenarios in
...
- [X] T012 [P] [US1] Unit tests for use-case services in
...
- [X] T013 [P] [US1] `@DataJpaTest` integration tests in
  `src/test/java/.../infrastructure/adapter/out/persistence/` for the client/project/task
  persistence adapters against H2.

### Implementation for User Story 1

- [X] T014 [P] [US1] `TaskStatus` enum in `src/main/java/.../domain/vo/TaskStatus.java`
  (TO_DO, IN_PROGRESS, DONE).
- [X] T015 [P] [US1] `Client` domain model in `src/main/java/.../domain/model/Client.java`.
- [X] T016 [P] [US1] `Project` domain model in `src/main/java/.../domain/model/Project.java`
  (holds clientId).
- [X] T017 [P] [US1] `Task` domain model in `src/main/java/.../domain/model/Task.java`
  (holds projectId, status; factory defaults TO_DO; `changeStatus` behavior).
- [X] T018 [P] [US1] Inbound port interfaces in `src/main/java/.../application/port/in/`:
  `ManageClientUseCase`, `ManageProjectUseCase`, `ManageTaskUseCase`.
- [X] T019 [P] [US1] Outbound port interfaces in `src/main/java/.../application/port/out/`:
  `ClientRepository`, `ProjectRepository`, `TaskRepository`.
- [X] T020 [US1] `ManageClientService` in `src/main/java/.../application/service/` (create/read/
  update/list; delete blocked when projects exist → `DependentRecordsException`).
- [X] T021 [US1] `ManageProjectService` in `src/main/java/.../application/service/` (validates
  client exists; delete blocked when tasks exist). Depends on T019, T020.
- [X] T022 [US1] `ManageTaskService` in `src/main/java/.../application/service/` (validates project
  exists; default TO_DO; status change; delete blocked when time entries exist). Depends on T019.
- [X] T023 [US1] JPA entities + Spring Data repositories + persistence adapters + mappers for
...
- [X] T024 [US1] Web controllers implementing generated `ClientsApi`, `ProjectsApi`, `TasksApi` in
...
- [X] T025 [P] [US2] Functional Gherkin scenarios in
...
- [X] T026 [P] [US2] Cucumber step definitions for time logging in
  `src/test/java/.../functional/steps/TimeLoggingSteps.java`.
- [X] T027 [P] [US2] Unit tests in `src/test/java/.../domain/TimeEntryTest.java` (duration > 0,
...
- [X] T028 [P] [US2] `@DataJpaTest` integration test for the time-entry persistence adapter in
  `src/test/java/.../infrastructure/adapter/out/persistence/TimeEntryPersistenceAdapterTest.java`.

### Implementation for User Story 2

- [X] T029 [P] [US2] `TimeEntryType` enum in `src/main/java/.../domain/vo/TimeEntryType.java`
  (BILLABLE, ADMINISTRATIVE).
- [X] T030 [P] [US2] `TimeEntry` domain model in `src/main/java/.../domain/model/TimeEntry.java`
  (taskId/projectId/clientId immutable, minutes > 0 invariant, default BILLABLE).
- [X] T031 [P] [US2] `LogTimeUseCase` inbound port in `src/main/java/.../application/port/in/` and
  `TimeEntryRepository` outbound port in `src/main/java/.../application/port/out/`.
- [X] T032 [US2] `LogTimeService` in `src/main/java/.../application/service/` (loads task, derives
  project+client, validates duration, defaults type BILLABLE, lists entries by task). Depends on
  T019 (TaskRepository), T031.
- [X] T033 [US2] JPA entity + Spring Data repository + persistence adapter + mapper for TimeEntry
  in `src/main/java/.../infrastructure/adapter/out/persistence/`. Depends on T008, T030, T031.
- [X] T034 [US2] Web controller implementing generated `TimeEntriesApi` (log + list by task) +
...
- [X] T035 [P] [US3] Functional Gherkin scenarios in
  `src/test/resources/features/hours-report.feature` (classify entries; report groups by type;
  same task contributes both types).
- [X] T036 [P] [US3] Cucumber step definitions in
  `src/test/java/.../functional/steps/HoursReportSteps.java`.
- [X] T037 [P] [US3] Unit tests in `src/test/java/.../application/service/`:
  `ClassifyTimeEntryServiceTest` and `HoursReportServiceTest` (aggregation by type + scope
  filtering).
- [X] T038 [P] [US3] `@DataJpaTest` integration test for the aggregation query in
  `src/test/java/.../infrastructure/adapter/out/persistence/HoursReportQueryTest.java`.

### Implementation for User Story 3

- [X] T039 [P] [US3] `HoursReport` derived model in `src/main/java/.../domain/model/HoursReport.java`
  (billable/administrative/total minutes + scope).
- [X] T040 [P] [US3] `ClassifyTimeEntryUseCase` and `HoursReportUseCase` inbound ports in
  `src/main/java/.../application/port/in/`.
- [X] T041 [US3] Add aggregation methods (sum minutes by type, filtered by client/project/task) to
...
- [X] T042 [US3] `ClassifyTimeEntryService` and `HoursReportService` in
  `src/main/java/.../application/service/`. Depends on T031, T040, T041.
- [X] T043 [US3] Web controllers: `PATCH /time-entries/{id}/classification` (extend TimeEntries
...
- [X] T045 [P] Run `./gradlew jacocoTestReport jacocoTestCoverageVerification`; add targeted tests
  to close any class below 80% or global below 80%.
- [X] T046 [P] Execute `specs/001-freelance-work-hub/quickstart.md` end-to-end against `bootRun`
  and confirm all validation-checklist items pass.
- [X] T047 Review for SOLID/DRY/YAGNI + Clean Architecture boundary violations (no framework
